 ...
 * QR-scan landing page. Returns null if the store id doesn't exist.
 */
export async function fetchPublicStoreInfo(
  ...
  const row = (data as unknown as
    | { display_name: string | null; google_review_url: string | null }[]
    | null)?.[0];
  if (!row) return null;
...
/**
 * Public (anon-callable): records a customer's phone number against a
 * store's reward program via the claim_store_reward security-definer RPC.
 * Points/status are fixed server-side (defaults on the node_rewards
 * table) — the client can't forge either.
 */
export async function claimStoreReward(
  storeId: string,
  phone: string,
): Promise<void> {
  const { error } = await supabase.rpc("claim_store_reward", {
    p_store_id: storeId,
    p_phone: phone,
  });
...
export async function fetchNodeRewards(
  ...
  const { data, error } = await supabase
    .from("node_rewards")
    .select("id, node_id, phone, points, status, created_at, claimed_at")
    .in("node_id", nodeIds)
...
/** Existing node_channels rows for a node (store), for prefilling the edit form. */
export async function fetchNodeChannels(
  nodeId: string,
): Promise<NodeChannelRecord[]> {
  const { data, error } = await supabase
    .from("node_channels")
    .select("id, node_id, channel_type, external_id, url, label, is_primary, status")
    .eq("node_id", nodeId);

  if (error) throw error;
  return ((data as unknown as NodeChannelRow[]) ?? []).map(toNodeChannelRecord);
}
